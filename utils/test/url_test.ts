...
assertEquals(
  { name: "冷方冰", age: "22", location: "beijing" },
  parseUrlQuery(url3).query,
);
assertEquals({}, parseUrlQuery(url).query);
assertEquals({ url: "/test.txt", extName: ".txt" }, parseExtname("/test.txt"));
assertEquals(
  { url: "/static/test.txt", extName: ".txt" },
  parseExtname("/static/test.txt"),
);
...
assertEquals(
  "domain=.foo.com;Path=/;secure;httpOnly",
  parseResponseCookie(
    { domain: ".foo.com", Path: "/", secure: true, httpOnly: true },
  ),
);
assertEquals(
  ["/name", { key: "", paramsName: "id" }, "/v1"],
  splitPath("/name/:id/v1"),
);
